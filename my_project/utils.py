...
def plot_location_epw_files(clustered_data=None):
    ...
    if clustered_data is not None:
        ...
        joined_df = pd.merge(df, clustered_data[['WBAN', 'prediction']], on='WBAN', how='inner')
        
        fig = px.scatter_mapbox(
            joined_df,#df,#df.head(2585),
            lat="Latitude",
            lon="Longitude",
            hover_name="Name",
    ...
    mapbox = dict(
        style='carto-positron',  # Puoi scegliere lo stile di mappa desiderato
        center=dict(lat=37, lon=-95),
        zoom=3,
        # Bounding box per l'America
        # Valori approssimativi per l'America settentrionale
        # Modifica i valori a seconda dell'area specifica che desideri visualizzare
        # Sud America: lat=12, lon=-80, Nord America: lat=72, lon=-168
        # Europa: lat=35, lon=-10, Asia: lat=45, lon=95, Africa: lat=0, lon=20
        # Australia: lat=-25, lon=135
        # Se vuoi visualizzare tutto il mondo, puoi rimuovere il parametro 'bounds'
        # e modificare il parametro 'zoom' per ottenere una visualizzazione completa
        bounds=dict(
            south=24,    # Latitudine minima (America meridionale)
            west=-125,   # Longitudine minima (Oceano Pacifico)
            north=50,    # Latitudine massima (Canada)
            east=-66     # Longitudine massima (Oceano Atlantico)
        )
    )
...
def plot_location_epw_files_clustering(k):    
    clustered_data = clustering(k)
    df = pd.read_csv('./assets/data/2013stationM.csv')
    joined_df = pd.merge(df, clustered_data[['WBAN', 'prediction']], on='WBAN', how='inner')
