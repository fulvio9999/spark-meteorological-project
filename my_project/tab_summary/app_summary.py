...
def update_map(meta):
    ...
    meta1 = {k:meta[k][0] if meta[k] is not None else None for k in meta} #BYFLV
    map_world = dcc.Graph(
        id="gh_rad-profile-graph",
        config=generate_chart_name("map", meta1),
...
def update_location_info(ts, df, meta, si_ip):
    ...
    # for k in meta:
    #     print(meta[k]) 
    # meta = {k:meta[k][0] if meta[k] is not None else None for k in meta}
    
    location_string = meta['city'][0]
    for i, loc in enumerate(meta['city'][1:]):
    ...
    location = f"Location: {location_string}"
    
    lon = f"Longitude: {meta1['lon']}"
    lat = f"Latitude: {meta1['lat']}"

    site_elevation = float(meta1["site_elevation"])
    ...
    if si_ip != "si":
        ...
        elevation = f"Elevation above sea level: {meta1['site_elevation']} m"
    
    period = ""
    if meta1["period"]:
        start, stop = meta1["period"].split("-")
        period = f"This file is based on data collected between {start} and {stop}"

    r = requests.get(
        f"http://climateapi.scottpinkelman.com/api/v1/location/{meta1['lat']}/{meta1['lon']}"
    )
    ...
    #BYFLV -------------------------------------------------------
    # total_solar_rad = (
    #     f"Annual cumulative horizontal solar radiation: NA "
    #     + total_solar_rad_unit
    # )
    # total_diffuse_rad = f"Percentage of diffuse horizontal solar radiation: NA %"
    # ----------------------------------------------------------
...
def degree_day_chart(ts, ts_click, df, meta, hdd_value, cdd_value, n_clicks, si_ip):
    """Update the contents of tab two. Passing in the general info (df, meta)."""
    meta = {k:meta[k][0] if meta[k] is not None else None for k in meta} #BYFLV
    # si_ip='si' #BYFLV
